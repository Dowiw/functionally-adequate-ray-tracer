...
static double	matrix_product(double *a, double *b, int row, int column, int size);

/*
 * Multiplies the given matrices 'a' and 'b'.
 * The product for each element in the resulting matrix is calculated by multiplying elements
 * in its row in matrix 'a' with elements in its column in matrix 'b' in sequence.
...
static void		matrix_multiply(double *result, double *a, double *b, int size)
{
	...
	while (r < size)
	{
		...
		while (c < size)
		{
			result[r * size + c] = matrix_product(a, b, r, c, size);
			c++;
		}
		r++;
	}
...
static double	matrix_product(double *a, double *b, int row, int column, int size)
{
	...
	while (i < size)
	{
		product += a[row * size + i] * b[i * size + column];
		i++;
	}
	return (product);
}
